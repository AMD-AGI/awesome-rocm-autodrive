...
__device__ void ms_deform_attn_col2im_bilinear(
    const scalar_t* __restrict__ bottom_data, const int &height,
    const int &width, const int &nheads, const int &channels, const scalar_t &h,
    const scalar_t &w, const int &m, const int &c, const scalar_t &top_grad,
    const scalar_t &attn_weight, scalar_t* __restrict__ grad_value,
  ...
  //   // total print [0, 0] 462 times
  //   printf("blockIdx.x %d, threadIdx.x %d: ptr1 = %d, ptr2 = %d, ptr3 = %d, ptr4 = %d\n", blockIdx.x, threadIdx.x, ptr1, ptr2, ptr3, ptr4);
  // }
...
__global__ void ms_deformable_col2im_gpu_kernel_shm_blocksize_aware_reduce_v1(
    const int n, const scalar_t* __restrict__ grad_col,
    ...
    const int num_point, scalar_t* __restrict__ grad_value,
    scalar_t* __restrict__ grad_sampling_loc,
    scalar_t* __restrict__ grad_attn_weight) {
  __shared__ scalar_t cache_grad_sampling_loc[blockSize * 2 + 1];
  __shared__ scalar_t cache_grad_attn_weight[blockSize + 1];
...
#pragma unroll
        for (int stride = blockSize / 2; stride > 0; stride >>= 1) {
          if (tid < stride) {
            cache_grad_attn_weight[tid] += cache_grad_attn_weight[tid + stride];
            cache_grad_sampling_loc[2 * tid] +=
                                    cache_grad_sampling_loc[2 * (tid + stride)];
            cache_grad_sampling_loc[2 * tid + 1] +=
                                cache_grad_sampling_loc[2 * (tid + stride) + 1];
          }
          __syncthreads();
        }
